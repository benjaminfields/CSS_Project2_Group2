...
# Load ypsps data
ypsps <- read_csv('CSS_Project2_Group2/ypsps.csv')
head(ypsps)
```
...
Iterations = 1000 # KQ: changing this to run simulations 
...
KQ: indeed, should be balanced on average across draws, i.e., a covariate will be balanced in expectation but there are extremes on the side of tails. by the way, it's generally better to reserve commenting for marking up code and instead write comments about your obserations and answers to the questions in markdown...like so... Also, there are a lot of plots here that don't convey much information. The most informative one about balance is probably the third one (the stacked barplot) and the others are a bit harder to make sense of in the context of covariate balance at baseline. One way to do this might have been to calculate the proportion of treated units with cov=1 / the total number of treated units, and then plot a histogram of those values. They should converge to .5 and you'll see imbalances on the tails. 
...
```{r}
# Drop columns with missing values and remove post-treatment covariates
ypsps <- ypsps %>% dplyr::select(-colnames(ypsps)[colSums(is.na(ypsps))>0])

#remove post-treatment covariates
ptc <- ypsps %>% dplyr::select(!contains("1973") & !contains("1982") & !"college" & !"interviewid")

# Create empty vectors to populate ATTs, proportion of balanced covariates, and mean percent balance improvement
ATT = rep(NA, Iterations)
balanced.ptc = rep(NA, Iterations)
balance_x = rep(NA, Iterations)

#empty list to hold model
ps_model_save = list()

# create vector of 10 random numbers from 1 to 10000
randomnumber = sample(1:Iterations, 10) 

#model stuff
for (i in 1:Iterations) {
  # randomly select covariates
  ptc.random = sample(ptc, size = sample(1:ncol(ptc)), replace=FALSE) 
  
  # run propensity score matching model, using nearest neighbor method, linear model, ATT estimand 
  ps.model <- paste("college ~", paste(names(ptc.random), collapse=" + "))
  ps_model = matchit(formula(ptc.random),
                           data = ypsps,
                           method = "nearest",
                           distance = "glm",
                           link = "logit",
  ...
  ps_model_save = summary(ps_model)
  
# Save ATTs, proportion of balanced covariates, and mean percent balance improvement
    att_data = match.data(ps_model)
    ps_outcome = paste("student_ppnscal ~ college +", paste(names(ptc.random), collapse=" + "))

    ps_att = lm(formula(ps_outcome), data = att_data, weights = weights)
    ps_summary = summary(ps_att)
    
    balance = bal.tab(ps_model, thresholds = .1)
    balance$Balanced.mean.diffs
    
    ATT[i] <- ps_summary$coefficients["college", "Estimate"]
    balanced.ptc[i] <- table(balance$Balance[-1, "M.Threshold"])["Balanced, <0.1"]/ncol(ptc.random) 
    balance_x[i] <- mean(ps_model$reduction[-1,1])
}
...
# PATT v. proportion
ggplot() + 
geom_point(aes(x=ATT, y=balanced.ptc))
# add regesssion line
...
# 10 random covariate balance plots
    if(i %in% rand.num){
      model_name <- paste0("model_", i)
        n <-  match(i, rand.num)
        plot(summary(psm_matchit, main= paste("Balance Model Plot", i)))
    }
...
# Create empty vectors to populate ATTs, proportion of balanced covariates, and mean percent balance improvement
iterations<-16 # KQ: what's going on here? why 16? 
...
# KQ: excellent and thorough response here 
...
# KQ: the answer to this question is a bit off. I'm not sure where the piece about decision trees conducting sensitivity analysis by design came from. Instead of defining terms, you might have talked about what propensity scores are doing and why accurately predicting selection into treatment might be more manageable with an ML algorithm. 
```
